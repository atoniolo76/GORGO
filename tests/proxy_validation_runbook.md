...
curl -s -X POST "$PROXY_URL/hyperparameters" \
  -H "Content-Type: application/json" \
  -d '{"t_prefill":0.8,"queued_tokens_weight":1.5}' | jq
```
...
[workload]   proxy hyperparameters: {'defaults': {'t_prefill': 0.8, 'queued_tokens_weight': 1.5}, 'per_target': {}}
[workload] hf dataset at '/datasets/datasets/lmsys__lmsys-chat-1m': 1000000 rows, using column 'conversation'
...
[workload]   proxy hyperparameters: {'defaults': {'t_prefill': 1.0, 'queued_tokens_weight': 1.0}, 'per_target': {}}
[workload] hf dataset at '/datasets/datasets/lmsys__lmsys-chat-1m': 1000000 rows, using column 'conversation'
...
```json
{
  ...
  "preview": {
    ...
    "recommendation": {
      "defaults": {
        "t_prefill": 0.0038264109391304576,
        "queued_tokens_weight": 0.006223890801575723
      },
      "per_target": {}
    }
  }
...
```text
[proxy] auto-tune #1 window=5 defaults={'t_prefill': 0.000839044623188541, 'queued_tokens_weight': 0.008014929686309102} per_target=[] (apply=False)
[proxy] auto-tune #2 window=5 defaults={'t_prefill': 0.002569623382429061, 'queued_tokens_weight': 0.007499608868239356} per_target=[] (apply=False)
[proxy] auto-tune #3 window=5 defaults={'t_prefill': 0.002976764047619033, 'queued_tokens_weight': 0.007236269025833169} per_target=[] (apply=False)
```
