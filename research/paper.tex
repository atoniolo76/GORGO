...
than black-box knobs. Results for \texttt{gorgo-autotune} on the main
production trace are omitted from main tables because per-replica sample counts
during the nighttime window are insufficient for stable rate fits; its behavior
on a higher-throughput short-prompt workload appears in Appendix~\ref{app:wildchat}.
...
\texttt{gorgo-autotune} results for GLM-5.1 are omitted from main tables;
per-replica sample counts during the nighttime window fall below the 16-sample
threshold for reliable median-of-rates fits, making the per-replica estimates
noisy. Its behavior on a high-throughput workload appears in Appendix~\ref{app:wildchat}. W1 supplies
...
\paragraph{gorgo-autotune sample requirements.}
The median-of-rates fit requires at least 16 completed requests routed to a given
replica before re-estimating $t_{\mathrm{prefill}}$ and $\alpha$. In low-throughput
nighttime windows (${\sim}1.2$ req/s split across three replicas and a 30-minute
observation window), the per-replica sample budget is thin, producing noisy rate
estimates and erratic routing. \texttt{gorgo-autotune} is therefore omitted from
the main production-trace tables; its behavior on a higher-throughput WildChat
workload is shown in Appendix~\ref{app:wildchat}. Future work could address this
by pooling samples across replicas or falling back to a global rate estimate when
per-replica counts are below threshold.
