...
% Review markup (added for the blueline pass). \chg = blue (earlier pass);
...
profiling. The GORGO family \rev{achieves the lowest TTFT p50 and p95 across both a tuning
window and a held-out evaluation window against all other load-balancing
policies; in the nighttime windows the p99 differences sit at the per-window
noise floor}, highlighting the impact of
...
\item \rev{Empirical results (\S\ref{sec:results}) across three 30-minute
  production-trace windows. The GORGO family achieves the lowest TTFT p50 and p95
  on the tuning and both held-out windows against the baseline policies, and
  sweeps all three percentiles in the midday window; the nighttime p99
  differences sit at the noise floor (\S\ref{sec:limitations}).}
\end{itemize}
...
Let a request $r$ with token sequence $x_r$ be routed to replica $i$. Its TTFT
decomposes as
\begin{equation}
\mathrm{TTFT}(r,i) = \mathrm{rtt}(i) + \mathrm{qt}(i,t_r)
  + \mathrm{pt}(i)\cdot|x_r \setminus c_i(t_r)| + \varepsilon,
\label{eq:ttft}
\end{equation}
where $\mathrm{rtt}(u)$ is the round-trip from the routing proxy to replica $i$;
$\mathrm{qt}(u,t_i)$ is queueing delay; $\mathrm{pt}(i)$ is
the per-token prefill rate; $c_i(t)$ is the set of token sequences cached on
$i$ at arrival time $t_r$, so $|x_r \setminus c_i(t_r)|$ is the number of uncached tokens. Cross-region $\mathrm{rtt}(i)$ ranges from 26\,ms to 466\,ms
...
\begin{equation}
\mathrm{score}(i, r) =
  w_{\mathrm{rtt}} \cdot \mathrm{rtt}_i
  + \left(|x_r| - |x_r \cap c_i|\right)
  + w_{\mathrm{queue}} \cdot \ell_i(t_r).
\label{eq:score}
\end{equation}
The proxy sends $r$ to the replica with the minimum score. Three signals feed
the score: (i) $\mathrm{rtt}_i$ from the Exponential Weighted Moving Average (EWMA) of a lightweight network probe
decoupled from the metrics scrape; (ii) $|x_r \cap c_i|$ from the proxy's local
radix trie of cached prefixes per replica, refreshed on every request completion;
and (iii) the count of tokens dispatched to each replica for all requests without a response, $\ell_i(t_r)$. 
The two free parameters are the RTT weight $w_{\mathrm{rtt}}$ and the queue weight $w_{\mathrm{queue}}$;
the own-prefill term is fixed as the unit of score.
...
as $\exp(\log(v) + \sigma\,\mathcal{N}(0,1))$ with $\sigma$ adapted according to
...
least-request             & 0.260          & 1.131          & \rev{\textbf{1.758}} & 1.23          & 2.16          & 2.90          & \textbf{8.1} & \textbf{127} & 100 \\
...
% -----------------------------------------------------------------------
\subsection{\rev{Why the load term is essential: TTFT-only tuning concentrates load}}
\label{sec:results:rewardhack}

\rev{The high learned $w_{\mathrm{load}}$ is not incidental --- it is what stops
the policy from reward-hacking the TTFT objective. Under continuous batching a
saturated replica still admits a new request's prefill and emits its first token
roughly on time, so \emph{TTFT is largely load-insensitive}; the decode backlog
instead dilutes the running batch and inflates end-to-end (E2E) latency. A policy
that optimizes TTFT alone therefore sees no penalty for herding every request
onto the single warmest replica. Left unconstrained, the ES finds exactly this
degenerate solution: it drives $w_{\mathrm{load}}$ to zero, routes 100\% of
traffic to one replica, and posts the \emph{best} TTFT on the nighttime trace,
while E2E p95 on the heavier midday trace explodes to 12.58\,s---$4.8\times$ worse
than \texttt{least-request} (Appendix~\ref{app:load_ablation}). Constraining the
load weight (which the ES then rails toward $w_{\mathrm{load}}{\approx}6.38$)
trades ${\sim}3\%$ TTFT p95 for ${\sim}5\times$ better E2E p95. The lesson is that
\textbf{E2E is the honest objective on a saturating fleet}: TTFT is the one axis
that does not price the decode backlog, so optimizing it alone optimizes for
hiding the cost.}

\rev{This tension is an artifact of co-locating prefill and decode on one
replica. Under prefill/decode disaggregation \citep{distserve2024,splitwise2024},
a request prefills on one pool and decodes on another, decoupling the decode
dilution from the prefill routing decision GORGO makes; the router would then
score only TTFT-relevant terms (RTT, uncached prefill, and prefill-pool queueing),
and concentrating prefill onto a cache-warm worker would no longer blow up decode
E2E. Characterizing GORGO on a disaggregated fleet is left to future work.}
...
0.07$\,s. \rev{In W2a the two best p99 latencies are \texttt{least-request}
(1.758\,s) and \texttt{simple-session-affinity} (1.785\,s)---a 0.027\,s gap, well
inside one SE---with \texttt{gorgo-static} at 1.844\,s; in W1 the best p99 is
\texttt{random} (1.996\,s). The nighttime p99 ordering among the top policies is
therefore not individually significant in either direction at this sample size,
and p50 and p95 carry the comparison. \texttt{gorgo-static} does sweep all three
percentiles in the heavier midday window (Table~\ref{tab:w2b}).}
...
(Table~\ref{tab:w2b}: \texttt{gorgo-hillclimb} p99 inflates to
...
\rev{achieves the lowest TTFT p50 and p95 across the tuning and held-out windows
(and sweeps all three percentiles in the heavier midday window), with the
nighttime p99 ordering between the top policies at the noise floor}. On a short-prompt, low-reuse public
...
Answer: \answerYes{}\\
Justification: The abstract claims (i) public datasets underrepresent long-context multi-turn traffic (quantified in \S\ref{sec:characterization}), (ii) we evaluate routing policies on a production trace (Tables~\ref{tab:w1}, \ref{tab:w2}, \ref{tab:w2b}), and (iii) \rev{GORGO achieves the lowest TTFT p50 and p95 in both 30-minute windows, and sweeps all three percentiles in the midday window}. We are explicit in the introduction, \S\ref{sec:results:p95}, and \S\ref{sec:limitations} that ``GORGO'' refers to a family, that the nighttime p99 ordering is at the noise floor, and that on the daytime stress window \texttt{gorgo-hillclimb}'s p99 inflates and \texttt{prefix-cache} wins p99 instead.
...
Answer: \answerYes{}\\
Justification: Section~\ref{sec:limitations} derives an approximate p99 standard error and \rev{shows that the W2a p99 margin between the top policies (\texttt{least-request} and \texttt{simple-session-affinity}) is inside one standard error, while the W1 and W2 p50/p95 wins are several standard errors clear of zero}.
