...
When $x'$ beats the parent weight $x_t$ on the objective metric, the incumbent weight $x_{t+1}$ is updated to $x'$, and $\sigma$ is adjusted to maintain \rev{Rechenberg's} 1/5 success rate of 1 accepted offspring for every 5 offspring \rev{\citep{rechenberg1973}}.
...
against five baseline policies.
\cmt{Confirmed: we are moving away from \texttt{gorgo-autotune}. It does not
appear in the main 2D results (Table~\ref{tab:results}); it survives only in the
WildChat appendix (Table~\ref{tab:wildchat}), where it was the unstable
rate-inversion variant. If dropping it, remove it here and from
Appendix~\ref{app:wildchat}.} \texttt{random} samples a replica uniformly per
...
convenience. Decode dynamics such as the effect of variable length decode and high memory consumption on load may impact live-tuning instability; exploring the impact of higher output token limits\rev{, or more sophisticated online or machine learning algorithms,} would be a direction for future work.
