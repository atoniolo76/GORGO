...
def main() -> None:
    ...
    best_tp = [_get(e["best_params"], "prefill_weight", "prefill_rate", default=0) for e in events]
    best_qw = [_get(e["best_params"], "load_weight", "load_rate", default=0) for e in events]
        ...
        if c:
            active_tp.append(
                _get(
                    ...
                    "prefill_weight",
                    "prefill_rate",
                    default=_get(bp, "prefill_weight", "prefill_rate", default=0),
                )
            )
            active_qw.append(
                _get(
                    ...
                    "load_weight",
                    "load_rate",
                    default=_get(bp, "load_weight", "load_rate", default=0),
                )
            )
            if has_rtt:
                active_rtt.append(c.get("rtt_weight", bp.get("rtt_weight", 1.0)))
        else:
            active_tp.append(_get(bp, "prefill_weight", "prefill_rate", default=0))
            active_qw.append(_get(bp, "load_weight", "load_rate", default=0))
            if has_rtt:
                active_rtt.append(bp.get("rtt_weight", 1.0))
    ...
    ax1.set_yscale("log")
    ax1.set_ylabel("prefill weight / rate", fontsize=12)
    ...
    ax2.set_yscale("log")
    ax2.set_ylabel("load weight / rate", fontsize=12)
